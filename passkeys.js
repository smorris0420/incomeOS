...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const { data: payBases } = await supabase.from('settings').select('user_id,value').eq('key','payBasis');
    const { data: salaries } = await supabase.from('settings').select('user_id,value').eq('key','annualSalary');
    const rateMap = {};
    ...
    const payBasisMap = {};
    (payBases||[]).forEach(r => { try { payBasisMap[r.user_id] = JSON.parse(r.value); } catch {} });
    const salaryMap = {};
    (salaries||[]).forEach(r => { try { salaryMap[r.user_id] = JSON.parse(r.value); } catch {} });
    return res.status(200).json((users||[]).map(u => ({...safeUser(u), defaultRate: rateMap[u.id] || null, employmentType: empMap[u.id] || 'full_time', payBasis: payBasisMap[u.id] || 'hourly', annualSalary: salaryMap[u.id] || null})));
  }
